...
async function handleRequest(req, res) {
  ...
  switch (resource) {
    ...
    case "notifications":
      return await handleNotifications(req, res, action);
    case "users":
 ...
 */
async function getRequests(req, res, supabase) {
  try {
    let query = supabase
 ...
 */
async function getTokens(req, res, supabase) {
  try {
    let query = supabase
      .from("dashboard_access_tokens")
      .select("*")
...
/**
 * Gestisce le richieste per le notifiche push
 */
...
async function sendNotification(req, res, supabase, webpush) {
  try {
    ...
    if (subError) {
      console.error("[Admin API] Errore recupero subscriptions:", subError);
      return res.status(500).json({ ok: false, error: "Errore recupero subscriptions" });
    }
    ...
    return res.status(200).json({
      ok: true,
      message: `Notifiche inviate: ${successful} riuscite, ${failed} fallite`,
      sent: successful,
      failed,
      total: subscriptions.length,
    });
  } catch (err) {
    console.error("[Admin API] Errore sendNotification:", err);
    return res.status(500).json({ ok: false, error: "Errore server", details: err.message });
  }
}

/**
 * Recupera le subscription per un utente o tutti gli utenti
 */
async function getNotifications(req, res, supabase, action) {
  try {
    if (action === "subscriptions") {
      const { user_id } = req.query || {};

      let query = supabase.from("push_subscriptions").select("id, user_id, created_at, updated_at");
      ...
      if (error) {
        console.error("[Admin API] Errore recupero subscriptions:", error);
        return res.status(500).json({ ok: false, error: "Errore recupero subscriptions" });
      }
    ...
    return res.status(400).json({ ok: false, error: "Action non valida" });
  } catch (err) {
    console.error("[Admin API] Errore getNotifications:", err);
    return res.status(500).json({ ok: false, error: "Errore server", details: err.message });
  }
}
