// /api/send-push.js
// API Vercel - Invio notifiche push (FCM + Web Push)
...
function initializeFirebaseAdmin() {
  ...
  try {
    ...
    console.error('[Send Push] Errore inizializzazione Firebase Admin:', err);
    throw err;
  }
}

// Inizializza Supabase per recuperare subscriptions
...
export default async function handler(req, res) {
  ...
  const apiKey = process.env.PUSH_API_KEY || 'your-secret-api-key'; // Configura questa variabile ambiente
  ...
  try {
    ...
    if (userIds && Array.isArray(userIds) && userIds.length > 0) {
      // Recupera subscriptions per utenti specifici
    ...
    // Configura web-push con VAPID keys da Firebase
    ...
    // Se abbiamo la chiave privata VAPID, configuriamo web-push
    if (VAPID_PRIVATE_KEY) {
      webpush.setVapidDetails(
        'mailto:tradelia@example.com', // Contact email (puoi cambiarlo)
        VAPID_PUBLIC_KEY,
        VAPID_PRIVATE_KEY
      );
    }
    ...
    const payload = JSON.stringify({
      ...
      data: {
        url: data?.url || '/archivio/dashboard.html',
        ...data
      }
    });
    ...
    const sendPromises = subscriptions.map(async (subscription) => {
      try {
        // Verifica formato subscription
        if (!subscription || !subscription.endpoint) {
          return { success: false, error: 'Subscription format invalid' };
        }

        // Se è una Web Push subscription (ha endpoint e keys)
        if (subscription.keys && subscription.keys.p256dh && subscription.keys.auth) {
          // Usa web-push library per Web Push notifications
          try {
            ...
            // Se web-push fallisce, prova con Firebase Admin SDK (se disponibile)
            console.warn('[Send Push] web-push fallito, prova FCM:', webPushErr.message);
            
            // Alternativa: converti subscription in formato FCM se possibile
            // Per ora ritorna errore
            return { success: false, error: webPushErr.message };
          }
        } else if (subscription.token) {
          // Se è un FCM token normale (non Web Push)
          try {
            initializeFirebaseAdmin();
        ...
        console.error('[Send Push] Errore invio singola push:', err);
    ...
    console.error('[Send Push] Errore:', err);
