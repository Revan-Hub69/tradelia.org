...
export function Header() {
  ...
  // Header sempre visibile - usa motion solo se client è pronto e motion è abilitato
  // Altrimenti mostra header statico per evitare scomparsa
  const shouldAnimate = isClient && !prefersReducedMotion;

  // Contenuto header - sempre lo stesso, indipendentemente da animazioni
  const headerContent = (
  ...
  // Se client è pronto e motion è abilitato, usa motion.header, altrimenti header statico
  // IMPORTANTE: Header sempre visibile, anche durante SSR/hydration
  if (shouldAnimate) {
    return (
      <motion.header
  ...
  // Header statico - sempre visibile, anche durante SSR/hydration
  return (
    <header
      className="sticky top-0 z-50 w-full border-b border-border-subtle glass supports-[backdrop-filter]:bg-bg-glass"
      suppressHydrationWarning
