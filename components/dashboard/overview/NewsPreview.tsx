...
export const NewsPreview = memo(function NewsPreview() {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      setLoading(true);
      try {
        const response = await fetch('/api/news/rss?limit=5');
        if (response.ok) {
    ...
    <section className="mb-6" aria-label="Anteprima notizie">
      <div className="mb-4">
        ...
        <div className="mt-2 text-right">
          <Link
            href={`${buildLocalePath(locale, '/dashboard/market-data')}#news-feed`}
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-semibold text-text-primary mb-2 line-clamp-2 group-hover:text-white transition-colors">
                    {item.title}
                  </h4>
                ...
                <ExternalLink className="w-4 h-4 text-text-tertiary group-hover:text-white transition-colors flex-shrink-0 mt-1" />
              </div>
            </a>
          ))}
        </div>
