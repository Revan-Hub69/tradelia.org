...
export async function GET(request: NextRequest) {
  try {
    ...
    const days = sanitizeNumberParam(searchParams.get('days'), 30, 1, 365);
    ...
    // Performance: Cache 1 minuto per dati real-time (custom)
    ...
    return createErrorResponse(
      error instanceof Error ? error : new Error('Failed to fetch historical levels'),
      500
    );
  }
}
