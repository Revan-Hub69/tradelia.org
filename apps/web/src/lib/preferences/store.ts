...
export async function loadPreferences(userId?: string) {
  ...
  type RemotePreferenceRow = {
    theme?: PreferencePayload['theme'] | null
    text_scale?: PreferencePayload['textScale'] | null
    animations?: PreferencePayload['animations'] | null
  }
  ...
  if (userId) {
    ...
    try {
      ...
      const prefs = (data ?? null) as RemotePreferenceRow | null
      if (!error && prefs) {
        remote = {
          theme: prefs.theme ?? 'dark',
          textScale: prefs.text_scale ?? 'normal',
          animations: prefs.animations ?? 'on',
        }
      }
    } catch (error) {
