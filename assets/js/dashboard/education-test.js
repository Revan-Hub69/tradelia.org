      ...
      <div class="question-options">
        ${question.education_question_options
          ...
          <label class="question-option" data-option-id="${option.id}">
            <input 
              type="${inputType}" 
              id="question-option-${questionId}-${option.id}"
              name="${inputName}" 
              value="${option.id}"
...
async function submitTest(testId) {
  ...
  const unanswered = Object.keys(currentTest.questions).filter((qId) => !testAnswers[qId]);

  if (unanswered.length > 0) {
    if (!confirm(`${unanswered.length} domande senza risposta. Vuoi inviare comunque il test?`)) {
      return;
    }
  }
