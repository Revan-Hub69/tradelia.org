...
export function calculateNextReviewDate(questionId, performance) {
  ...
  const daysSinceLastReview = Math.floor((Date.now() - lastReview.getTime()) / (1000 * 60 * 60 * 24));
  ...
  if (successRate < 0.5) {
    category = "incorrect";
  } else if (successRate < 0.7 || avgTime > 120000) { // > 2 minuti
    category = "difficult";
...
function renderQuestionCard(question, isDueToday) {
  ...
  const difficultyColor = successRate < 0.5 ? "var(--edu-error)" : 
                         successRate < 0.7 ? "var(--edu-warning)" : 
                         "var(--edu-success)";
...
async function startReviewSession(questionId) {
  try {
    ...
    window.history.pushState({ view: "retrieval-practice", questionId }, "", `#education/review/${questionId}`);
