...
export class InteractiveTools {
  ...
  calculateCosts() {
    ...
    const trackingError = parseFloat(document.getElementById("tracking-error-input")?.value || 0.15) / 100;
      ...
      resultsContainer.innerHTML = `
        <div class="cost-results">
          ...
          <div class="result-item ${totalCosts > 0.005 ? "warning" : "success"}">
            ...
            <strong>${totalCosts > 0.005 ? "⚠️ Costi elevati (>0.5%)" : "✅ Costi ottimali (<0.5%)"}</strong>
          </div>
        </div>
      `;
    }
