...
function getToneClass(tone) {
  switch (tone) {
    case 'ok':
      return 'metric-tone--ok';
    case 'warn':
      return 'metric-tone--warn';
    case 'err':
      return 'metric-tone--err';
    default:
...
function renderFilters(container) {
  ...
  toneFilters.forEach((filter) => {
    const btn = createEl(
      'button',
      `drawer-filter-btn ${DRAWER._activeFilter === filter.value ? 'active' : ''}`,
      filter.label
    );
...
function renderGroup(groupData) {
  ...
  groupData.metrics.forEach((metric) => {
    ...
    const item = createEl(
      'button',
      `drawer-metric-item ${getToneClass(metric.tone)} ${isSelected ? 'drawer-metric-selected' : ''}`
    );
  ...
  if (DRAWER._selectedMetricKey) {
    setTimeout(() => {
      const selectedItem = container.querySelector(
        `[data-metric-key="${DRAWER._selectedMetricKey}"]`
      );
      if (selectedItem) {
...
function applyFilters() {
  DRAWER._filteredMetrics = filterMetrics(
    DRAWER._allMetrics,
    DRAWER._searchQuery,
    DRAWER._activeFilter
  );
  renderView();
}
