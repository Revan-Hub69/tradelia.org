  ...
  function openDrawer(tabId, tabTitle) {
    ...
    unifiedDrawer.open({
      ...
      setTimeout(() => {
        // Cerca il drawer content (può essere dentro unified-drawer)
        const drawerContent = document.querySelector('.glossary-drawer-content.module-tabs-content-wrapper') ||
                             document.querySelector('.unified-drawer-content') ||
                             document.querySelector('[class*="drawer-content"]');
        if (drawerContent) {
          // Cerca container per ticker (altre tab) o chart (tab Chart)
          // Supporta sia data-tab-ticker="..." che data-tab-charts="true"
          const tickerContainer = drawerContent.querySelector('[data-tab-ticker]');
          const chartContainer = drawerContent.querySelector('[data-tab-charts]') ||
                                drawerContent.querySelector('[data-tab-charts="true"]');
          const container = chartContainer || tickerContainer;
          ...
          if (container) {
            ...
            // Debug: se non troviamo il container, proviamo a cercarlo nel contenuto appena caricato
            console.warn('Container non trovato nel drawer, tabId:', tabId, 'drawerContent:', drawerContent, 'content:', drawerContent?.innerHTML?.substring(0, 200));
          }
        } else {
          console.warn('Drawer content non trovato');
        }
        isOpening = false;
      }, 150); // Delay di 150ms per assicurare che il drawer sia renderizzato
  }
...
>>>>>>> ec4ba64 (fix: migliorato supporto chart F1B in module-header.js e f1b.js)
