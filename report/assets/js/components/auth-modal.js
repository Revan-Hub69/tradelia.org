...
async function handleLogin(event) {
  ...
  // Best practice: email format validation
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailRegex.test(email)) {
    showFieldError(form.querySelector('#auth-email-login'), 'Inserisci un indirizzo email valido');
    return;
  }
  state.busy = true;
  const submitBtn = form.querySelector('button[type="submit"]');
  ...
  try {
    ...
    if (/api key/i.test(message)) {
      showToast(
        'Configurazione API non valida. Verifica la Supabase anon key sul deploy.',
        'error'
      );
    }
  } finally {
...
async function handleSignup(event) {
  ...
  if (password.length < 8) {
    showFieldError(
      form.querySelector('#auth-password-register'),
  ...
  try {
    ...
    if (session && session.user) {
      Logger.debug('AuthModal', 'Logout account esistente prima di signup', {
    ...
    try {
      ...
      if (profileError) {
        ...
        if (profileError.code === '23505') {
          ...
          console.error(
            '[AuthModal] RLS Policy Error - Verifica che le policy per user_profiles permettano INSERT durante signup'
          );
        } else if (
          ...
          console.error(
            '[AuthModal] ERRORE CRITICO: Tabella user_profiles non esiste. Esegui supabase/setup-complete-schema.sql'
          );
        } else {
    ...
    if (requiresEmailVerification) {
      ...
      if (!signInError && signInData?.user) {
        Logger.debug('AuthModal', 'Auto-login riuscito dopo signup', {
    ...
    if (
      ...
      errMsgLower.includes('error sending email') ||
      errMsgLower.includes('email confirmation')
    ) {
      errorMessage =
        'Account creato ma email di verifica non inviata. Controlla la configurazione SMTP in Supabase. Puoi comunque accedere.';
      ...
      errMsgLower.includes('insufficient privilege')
    ) {
      errorMessage =
        'Errore di permessi. Esegui lo script supabase/setup-complete-schema.sql in Supabase per configurare le RLS policies.';
    } else if (
      errCode === '42P01' ||
      errMsgLower.includes('does not exist') ||
      errMsgLower.includes('non esiste')
    ) {
      errorMessage =
        'Tabelle mancanti. Esegui lo script supabase/setup-complete-schema.sql in Supabase SQL Editor.';
    } else if (
      errCode === '23514' ||
      errMsgLower.includes('check constraint') ||
      errMsgLower.includes('violates check constraint')
    ) {
      errorMessage =
        'Errore constraint. Esegui supabase/migration-add-guest-role.sql per aggiungere il ruolo "guest".';
    } else if (
      err.status === 500 ||
      errCode === 'PGRST' ||
      errMsgLower.includes('unexpected_failure')
    ) {
      // Errore 500 da Supabase Auth - probabilmente SMTP o configurazione
      if (
        errMsgLower.includes('email') ||
        errMsgLower.includes('smtp') ||
        errMsgLower.includes('mail')
      ) {
        errorMessage =
          'Errore invio email. Disabilita email verification in Supabase (Auth → Settings) oppure configura SMTP correttamente.';
      } else {
        errorMessage =
          'Errore server Supabase. Verifica: 1) Email verification disabilitata O 2) SMTP configurato correttamente. Vedi SUPABASE-500-SIGNUP-FIX.md';
      }
    }
...
async function handleReset(event) {
  ...
  if (!email) {
    showFieldError(
      form.querySelector('#auth-email-reset'),
      "Inserisci la mail associata all'account."
    );
    return;
  }
  ...
  try {
    ...
    showToast(
      'Email inviata! Controlla la tua casella email (anche spam) e clicca sul link per reimpostare la password.',
      'success'
    );
    ...
    if (err.message) {
      ...
      if (
        ...
        errorMessage =
          'Troppe richieste di reset password. Attendi qualche minuto prima di riprovare.';
      } else if (
        errMsgLower.includes('email') ||
