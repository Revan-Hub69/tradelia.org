...
function open(config) {
  ...
  if (UNIFIED_DRAWER._content) {
    ...
    if (typeof content === 'string') {
      ...
      if (
        !content.includes('glossary-drawer-search') &&
        !content.includes('glossary-drawer-filters')
      ) {
    ...
    if (!UNIFIED_DRAWER._content.__metricClickBound) {
      ...
      UNIFIED_DRAWER._content.addEventListener(
        'click',
        async (e) => {
        ...
        { capture: true }
      ); // Usa capture per intercettare prima che altri handler possano interferire
    }
  }
  ...
  registerOverlay(
    UNIFIED_DRAWER._overlayId,
    OVERLAY_TYPES.MODULE_TABS,
    UNIFIED_DRAWER._overlay,
    UNIFIED_DRAWER._panel
  );
