...
function normalizeDataPublicF5B(src = {}) {
  ...
  const defaults = {
    ...
    hero_desc:
      'Analisi educativa di strutture opzioni esemplificative (CALL/PUT, spread), coerenti con bias e rischio. Nessun contenuto operativo o raccomandativo.',
...
function generateAISummaryRows(data) {
  ...
  rows.push({
    ...
    parts: [
      {
        kind: 'text',
        text: `${labels.label_bias_option || 'Orientamento Strutturale'}${labels.separator_colon || ': '}`,
      },
      {
        kind: 'metric',
        key: 'BiasOption',
      ...
      {
        kind: 'text',
        text: `${labels.separator_dot || ' · '}${labels.label_iv_regime || 'Regime Volatilità Implicita'}${labels.separator_colon || ': '}`,
      },
      {
        kind: 'metric',
        key: 'IV_regime',
  ...
  if (setupScore !== '—') {
    rows.push({
      ...
      parts: [
        {
          kind: 'text',
          text: `${labels.label_setup_score || 'Score Configurazione F5B'}${labels.separator_colon || ': '}`,
        },
        {
          kind: 'metric',
          key: 'SetupScore_F5B',
