...
function normalizeDataPublicF2(src = {}) {
  ...
  const defaults = {
    ...
    hero_desc:
      'Contesto sintetico non operativo su sentiment, fondamentali descrittivi e newsflow.',
...
function generateAISummaryRows(data) {
  ...
  const sentiment =
    d.sentiment_flows?.SentimentComposite?.raw || d.sentiment_flows?.SentimentComposite || '—';
  ...
  rows.push({
    ...
    parts: [
      {
        kind: 'text',
        text: `${labels.label_macrogate || 'MacroGate'}${labels.separator_colon || ': '}`,
      },
      {
        kind: 'metric',
        key: 'MacroGate',
      ...
      {
        kind: 'text',
        text: `${labels.separator_dot || ' · '}${labels.label_sentiment_composite || 'Sentiment'}${labels.separator_colon || ': '}`,
      },
      {
        kind: 'metric',
  ...
  if (etfFlow !== '—') {
    rows.push({
      ...
      parts: [
        {
          kind: 'text',
          text: `${labels.label_etf_flow_tone || 'ETF Flussi'}${labels.separator_colon || ': '}`,
        },
        {
          kind: 'metric',
          key: 'ETF_FlowTone',
