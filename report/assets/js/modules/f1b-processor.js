  ...
  return {
    ...
    f1bSnapshot: {
      regime_state: {
        ...
        RegimeScore_definition: "Score [-1..+1] appetito rischio sintetico (vol, credito, curva, equity beta). >0 = risk-on."
      },
      ...
      size_distribution: {
        ...
        SizeBias_definition: "Flusso relativo ultimi 5-20g tra mega, large, mid, small, micro, nano.",
        StressMicroCap: breadthMetrics.StressMicroCap || false,
        StressMicroCap_definition: "true = microcap in drawdown e funding risk alto → da escludere dal universe swing.",
      ...
      risk_window: {
        ...
        RiskWindow_definition: "Snapshot macro sensibile all'orizzonte 3–10 giorni (vol implicita, costo capitale, shock energia, calendario evento)."
      }
    },
    ...
    mifid: {
      disclaimer: "Questo contenuto ha finalità esclusivamente educativa e informativa e non costituisce in alcun modo raccomandazione d'investimento o consulenza personalizzata ai sensi della normativa MiFID II."
    }
  };
}
...
function calculateRegimeMetrics(rawData) {
  ...
  const regimeScore = Math.max(-1, Math.min(1, 
    volComponent + creditComponent + curveComponent + (breadthComponent - 0.5) * 0.3
  ));
...
function calculateBreadthMetrics(rawData) {
  ...
  const greenSectors = Object.values(sectors1M).filter(v => 
    (typeof v === 'number' && v > 0) || 
    (typeof v === 'object' && v.performance > 0)
  ).length;
  const breadth1M = totalSectors > 0 ? greenSectors / totalSectors : 0.5;

  // RiskTilt_1M: pro-rischio vs difensivo
  const growthSectors = ['Technology', 'Communication Services', 'Consumer Discretionary', 'Industrials'];
  const defensiveSectors = ['Utilities', 'Consumer Staples', 'Healthcare', 'Real Estate'];
  
  const growthAvg = growthSectors.reduce((sum, s) => {
  ...
  const defensiveAvg = defensiveSectors.reduce((sum, s) => {
    const perf = sectors1M[s]?.performance || sectors1M[s] || 0;
    return sum + (typeof perf === 'number' ? perf : perf.performance || 0);
  ...
  const sectorPerformance = Object.entries(sectors1M).map(([name, perf]) => ({
    name,
  ...
  return {
    ...
    DefensiveLeadership: defensiveSectors.filter(s => 
      sectorPerformance.find(sp => sp.name === s)?.perf > 0
    ),
    ...
    SizeBiasPattern: sizeBias === "MegaCap" 
...
function getStrategyModeDefinition(mode) {
  const definitions = {
    "Momentum": "Momentum = vol implicita giù, leadership growth/ciclici, curva tassi non in stress; condizioni favorevoli al rischio.",
    "Momentum-light": "Momentum-light = trend moderato, condizioni intermedie, rotazioni settoriali.",
    "Pullback": "Pullback = fase di scarico, volatilità alta, breadth ristretta, preferenza per qualità.",
 ...
 */
export function generateFinvizFilters(strategyMode, leadersMultiTF = [], sizeBias = "Mixed", stressMicroCap = false) {
...
function buildBridgeF2(breadthMetrics, finvizFilters, regimeMetrics, strategyMode) {
  ...
  return {
    universe_for_F2: {
      ...
      FinvizQuery_definition: "Filtro da applicare alla sorgente di screening per produrre la lista ticker candidata."
    },
...
function convertToLegacyFormat(regimeMetrics, strategyMode) {
  const tone = regimeMetrics.RegimeScore > 0.3 ? "green" : 
               regimeMetrics.RegimeScore < -0.2 ? "red" : "yellow";

  return {
    StrategyMode_macro: {
      raw: strategyMode,
      tone: strategyMode === "Momentum" ? "green" : 
            strategyMode === "Pullback" ? "red" : "yellow",
      ai_note: getStrategyModeDefinition(strategyMode)
    },
    RegimeScore: {
      raw: regimeMetrics.RegimeScore >= 0 ? `+${regimeMetrics.RegimeScore.toFixed(2)}` : regimeMetrics.RegimeScore.toFixed(2),
      tone: tone,
    ...
    LiquidityRegimeScore: {
      raw: regimeMetrics.LiquidityRegimeScore >= 0 ? `+${regimeMetrics.LiquidityRegimeScore.toFixed(2)}` : regimeMetrics.LiquidityRegimeScore.toFixed(2),
      tone: tone,
      ai_note: ""
    },
    CreditRiskBlock: {
...
function convertBreadthToLegacyFormat(breadthMetrics) {
  return {
    ...
    SmallCapPressure_1W: {
      raw: breadthMetrics.SmallCapPressure_1W >= 0 ? `+${breadthMetrics.SmallCapPressure_1W.toFixed(2)}` : breadthMetrics.SmallCapPressure_1W.toFixed(2),
      tone: "yellow",
      ai_note: ""
    },
    IndexMomentum_1W: {
...
export const handoffGuidance = {
  F2_expectations: "F2 deve prendere universe_for_F2, applicare la query Finviz/scan quantitativo, calcolare i ranking proprietari (SCI/IPI/ICR) SOLO sui ticker risultanti. Escludere microcap se StressMicroCap=true.",
  F3_expectations: "F3 (Analisi Tecnica) deve usare StrategyMode_macro e RiskWindow_F1. Se StrategyMode_macro='Momentum' e RiskWindow_F1.Score>0, privilegia pattern di continuazione/breakout; se 'Pullback', privilegia mean reversion.",
  F4_expectations: "F4 (Intermarket) deve confrontare RiskWindow_F1.Commodities con la sensibilità settoriale: esempio oil spike→penalità airline/trasporti.",
  F5_expectations: "F5 (Gestione Dinamica) usa RegimeScore per sizing di rischio. Valori bassi => sizing più contenuto. (Solo interno desk, non pubblico / non MiFID.)",
  F6_expectations: "F6 (Gestione Dinamica) tiene monitorati: VIX_level, Curve_state.CurveShape_comment, StressMicroCap. Se StressMicroCap passa da true a false, riapre eventualmente small/smaller cap nell'universo.",
  F7_audit_note: "Tutti i moduli successivi devono riportare auditPathID e timestampET per tracciabilità. Nessuna deduzione soggettiva o previsionale senza dato esplicito Tier-1.",
  governanceFlags: {
    MiFID_public: "NON includere handoffGuidance, FinvizQuery o sizing fuori dall'ambiente interno.",
