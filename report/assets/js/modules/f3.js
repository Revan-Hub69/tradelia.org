...
// Allineato a F1B/F2: stesso card system, stessa tassonomia UI, nessun testo narrativo hard-coded.
//
// Export:
      ...
      <header class="section-headline mb-4">
        <div class="section-head-left">
          ...
          <div class="section-desc text-[12px] text-[color:var(--muted)] leading-[1.45] mt-1">
            ...
            <br/>
            <span class="text-[11px] text-[color:var(--muted)]">${escapeHtml(d.meta.hero_disclaimer)}</span>
          </div>
        </div>
      </header>
...
export function bindCard(node, rawData, ctx = {}) {
  ...
  // Delegation per CTA (coerente con F1/F2)
...
function openF3DrawerPublic(d) {
  ...
  setTimeout(() => {
    ...
    if (window.__TradeliaUI?.bindMetricInfoButtons) {
      try { window.__TradeliaUI.bindMetricInfoButtons(root?.querySelector('#f3-scroll-desktop')); } catch {}
      try { window.__TradeliaUI.bindMetricInfoButtons(root?.querySelector('#f3-scroll-mobile')); } catch {}
    }

    const first = root?.querySelector('[data-f3-tab="dataset"]');
    if (first && typeof first.click === 'function') first.click();

    // Hint scroll tabs (scoped)
    ...
    if (scrollBox && fadeRight) {
      ...
      if (!needs) {
        ...
        const fl = root?.querySelector('.f1b-tabs-fade-left'); if (fl) fl.style.display = 'none';
      } else {
        update();
...
/* -----------------------------------------------------------------------------
   SEZIONI (solo dati da JSON, nessuna narrativa hard-coded)
----------------------------------------------------------------------------- */
function buildF3SectionsPublic(d) {
  ...
  <section class="tl-panel-section" data-f3-section="dataset" style="background:transparent;border:0;border-radius:0;box-shadow:none;padding:0;">
    ...
    ${metricBlockF3('OCR_Conf','OCR Confidence','', d.dataset?.OCR_Conf)}
    ${metricBlockF3('DataIntegrity','Data Integrity','', d.dataset?.DataIntegrity)}
    ${metricBlockF3('FeedSync','Feed Sync','', d.dataset?.FeedSync)}
    ${metricBlockF3('SessionScope','Session Scope','', d.dataset?.SessionScope)}
  ...
  <section class="tl-panel-section" data-f3-section="w1" style="background:transparent;border:0;border-radius:0;box-shadow:none;padding:0;">
    ...
    <div class="grid md:grid-cols-2 gap-3 text-[12px] leading-[1.4]">
      ${metricBlockF3('RSI_W1','RSI(14)','', w1?.RSI14)}
      ${metricBlockF3('ADX_W1','ADX(14)','', w1?.ADX14)}
      ${metricBlockF3('MACD_W1','MACD(12,26,9)','', w1?.MACD)}
      ${metricBlockF3('EMA_W1','EMA 9/20/50/200','', w1?.EMA)}
      ${metricBlockF3('EMAX_W1','EMA Cross Ratio','', w1?.EMA_CrossRatio)}
      ${metricBlockF3('BBWidth_W1','BBWidth%','', w1?.BBWidthPct)}
      ${metricBlockF3('ATR_W1','ATR% (14)','', w1?.ATRpct14)}
      ${metricBlockF3('HV_W1','HV(30/90)','', w1?.HV_30_90)}
      ${metricBlockF3('VOL_W1','Volumi/Flusso','', w1?.VOL_BLOCK)}
      ${metricBlockF3('PAT_W1','Pattern volumetrici (estratto)','', w1?.PATTERNS_SUMMARY)}
    </div>
  ...
  <section class="tl-panel-section" data-f3-section="d1" style="background:transparent;border:0;border-radius:0;box-shadow:none;padding:0;">
    ...
    <div class="grid md:grid-cols-2 gap-3 text-[12px] leading-[1.4]">
      ${metricBlockF3('RSI_D1','RSI(14)','', d1?.RSI14)}
      ${metricBlockF3('ADX_D1','ADX(14)','', d1?.ADX14)}
      ${metricBlockF3('MACD_D1','MACD(12,26,9)','', d1?.MACD)}
      ${metricBlockF3('EMAX_D1','EMA Cross Ratio','', d1?.EMA_CrossRatio)}
      ${metricBlockF3('BBW_D1','BBWidth%','', d1?.BBWidthPct)}
      ${metricBlockF3('ATR_D1','ATR% (14)','', d1?.ATRpct14)}
      ${metricBlockF3('VOLZ_D1','Volume z / RVOL','', d1?.VOL_CLUSTER)}
      ${metricBlockF3('OBV_D1','OBV/CMF/MFI/PVT','', d1?.FLOW_SET)}
      ${metricBlockF3('DONCH_D1','Donchian(20)','', d1?.Donchian20)}
      ${metricBlockF3('PAT_D1','Pattern volumetrici (estratto)','', d1?.PATTERNS_SUMMARY)}
    </div>
  ...
  <section class="tl-panel-section" data-f3-section="h4" style="background:transparent;border:0;border-radius:0;box-shadow:none;padding:0;">
    ...
    <div class="grid md:grid-cols-2 gap-3 text-[12px] leading-[1.4]">
      ${metricBlockF3('RSI_H4','RSI(14)','', h4?.RSI14)}
      ${metricBlockF3('ADX_H4','ADX(14)','', h4?.ADX14)}
      ${metricBlockF3('CMF_H4','CMF(20)','', h4?.CMF20)}
      ${metricBlockF3('OBVS_H4','OBV slope','', h4?.OBV_slope)}
      ${metricBlockF3('VOLZ_H4','Volume z','', h4?.Volume_z)}
      ${metricBlockF3('CONF_H4','Confidence H4','', h4?.Confidence_H4)}
    </div>
    ${headlineBlockCardF3('Interpretazione H4', h4?.ai_note)}
  </section>`;
  ...
  <section class="tl-panel-section" data-f3-section="h1" style="background:transparent;border:0;border-radius:0;box-shadow:none;padding:0;">
    ...
    <div class="grid md:grid-cols-2 gap-3 text-[12px] leading-[1.4]">
      ${metricBlockF3('RSI_H1','RSI(14)','', h1?.RSI14)}
      ${metricBlockF3('VOLZ_H1','Volume z','', h1?.Volume_z)}
      ${metricBlockF3('BBW_H1','BBWidth%','', h1?.BBWidthPct)}
      ${metricBlockF3('ATR_H1','ATR% (1h)','', h1?.ATRpct)}
      ${metricBlockF3('CONF_H1','Confidence H1','', h1?.Confidence_H1)}
    </div>
    ${headlineBlockCardF3('Interpretazione H1', h1?.ai_note)}
  </section>`;
  ...
  <section class="tl-panel-section" data-f3-section="options" style="background:transparent;border:0;border-radius:0;box-shadow:none;padding:0;">
    ...
    ${renderOptionsSectionPublic(d.options)}
  </section>`;
  ...
  <section class="tl-panel-section" data-f3-section="price" style="background:transparent;border:0;border-radius:0;box-shadow:none;padding:0;">
    ...
    <div class="grid md:grid-cols-2 gap-3 text-[12px] leading-[1.4]">
      ${metricBlockF3('RANGE','Range%','', ph?.RangePct_60D)}
      ${metricBlockF3('GAPS','Gap% (stat)','', ph?.GapPct_stats)}
      ${metricBlockF3('STREAKS','Streaks (run)','', ph?.Streaks_stats)}
      ${metricBlockF3('RVOL_CLUST','Cluster RVOL','', ph?.RVOL_clusters)}
    </div>
    ${headlineBlockCardF3('Note prezzo', ph?.ai_note)}
  </section>`;
  ...
  <section class="tl-panel-section" data-f3-section="patterns" style="background:transparent;border:0;border-radius:0;box-shadow:none;padding:0;">
    ...
    ${patternBoardF3(d.pattern_mtf)}
    ${headlineBlockCardF3('Note pattern', d.pattern_mtf?.ai_note_pattern)}
  </section>`;

  // 9) Consolidamento MTF & Probabilità
  ...
  <section class="tl-panel-section" data-f3-section="mtf" style="background:transparent;border:0;border-radius:0;box-shadow:none;padding:0;">
    ...
    <div class="grid md:grid-cols-2 gap-3 text-[12px] leading-[1.4]">
      ${metricBlockF3('COMP_MTF','Compositi MTF (Vol/Opt/Price)','', mtf?.Composites_MTF)}
      ${metricBlockF3('CBI','CBI','', mtf?.CBI)}
      ${metricBlockF3('TE','TE (Trend Efficiency)','', mtf?.TE)}
      ${metricBlockF3('DPI','DPI','', mtf?.DPI)}
      ${metricBlockF3('MVA','MVA_hybrid','', mtf?.MVA_hybrid)}
      ${metricBlockF3('PROB','Probabilità (Swing/LT/Breakout)','', mtf?.Probabilities)}
      ${metricBlockF3('MTFS','MTF Score','', mtf?.MTF_Score)}
      ${metricBlockF3('BIAS','Bias MTF','', mtf?.Bias_MTF)}
      ${metricBlockF3('CONF_FINAL','Confidence finale','', mtf?.Confidence_final)}
    </div>
    ${headlineBlockCardF3('Sintesi MTF', mtf?.ai_note_mtf)}
  </section>`;
...
/* -----------------------------------------------------------------------------
   Options Overlay (solo presentazione; tutto da JSON)
----------------------------------------------------------------------------- */
function renderOptionsSectionPublic(o = {}) {
  ...
  const kpiGrid = `
    <div class="grid gap-3 grid-cols-2 md:grid-cols-4">
      ${metricBoxTrafficLightF3({ key:'IV_ATM',  label:'IV (ATM)',          desc:'', metric:o.IV_ATM })}
      ${metricBoxTrafficLightF3({ key:'IV_rank', label:'IV Rank / %tile',   desc:'', metric:o.IV_rank_pct })}
      ${metricBoxTrafficLightF3({ key:'PCR',     label:'Put/Call (Vol/OI)', desc:'', metric:o.PutCall })}
      ${metricBoxTrafficLightF3({ key:'GSR',     label:'GSR (Γ/Vega)',      desc:'', metric:o.GSR_tkr })}
    </div>`;
  cards.push(kpiGrid);

  // Expected Move (tabella semplice)
  const emRows = Array.isArray(o.EM_rows) ? o.EM_rows.slice(0, 8).map(r => `
    <tr>
  ...
  // PCR breakdown (se presente)
  const pcr = o.PCR_breakdown || {};
  const pcrRight = Array.isArray(pcr.by_expiry)
    ? pcr.by_expiry.slice(0,4).map(x =>
        `<div class="text-[12px] font-mono">${escapeHtml(String(x?.exp ?? '—'))} · EM ${fmtPct(x?.em)} · PCRv ${fmtNum(x?.pcr_v)} · PCRoi ${fmtNum(x?.pcr_oi)}</div>`
      ).join('')
    : '';

  if (pcr.total_vol !== undefined || pcrRight) {
    cards.push(f3Card({
      tone: 'neutral',
  ...
  // Gamma / Max Pain
  if (o.GEX || o.DealerGamma || o.MaxPain) {
    const gex = o.GEX || {};
    cards.push(f3Card({
      tone: 'neutral',
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <div>
          <div class="text-[12px]">Max Pain <b>${fmtNum(o?.MaxPain?.strike)}</b> ${o?.MaxPain?.exp ? `(exp ${escapeHtml(o.MaxPain.exp)})` : ''}</div>
  ...
  // Skew 25Δ
  if (o.Skew_detail || o.Skew_set || o.ai_note_skew) {
  ...
  if (o.Flow) {
    const flow = o.Flow || {};
    const top = Array.isArray(flow.top)
      ? flow.top.slice(0,3).map(t =>
    ...
    cards.push(f3Card({
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <div>${top || '<div class="text-[12px] text-[color:var(--muted)]">—</div>'}</div>
      </div>`
    }));
  }

  // Term structure
  if (o.TermSlope || o.TermNotes) {
    cards.push(f3Card({
  ...
  // GSR board
  if (o.GSR_tkr || o.Gamma_ATM || o.Vega_ATM || o.ai_note_gsr) {
    cards.push(f3Card({
  ...
  // Data gaps
  if (Array.isArray(o.DataGaps) && o.DataGaps.length) {
    cards.push(listBlockCardF3('Data gaps (Options)', o.DataGaps));
  }

  // AI summary
  if (o.ai_note) {
    cards.push(headlineBlockCardF3('AI Summary (Options)', o.ai_note));
  }
...
/* -----------------------------------------------------------------------------
   CARD BUILDERS (coerenti con F1B/F2)
----------------------------------------------------------------------------- */
...
function metricBlockF3(metricKey, title, desc, metricObj) {
  ...
  const valueHtml = renderValueSmart(metricObj);
  const bodyHtml = `
    <div class="flex items-start justify-between gap-2 mb-1">
      <div class="flex items-center gap-2">
        <span class="font-mono font-bold text-[13px] leading-[1.4]" style="color:${textColor};">${valueHtml}</span>
      </div>
      <button class="info-btn" data-metric="${escapeAttr(metricKey)}_info" aria-label="Info ${escapeAttr(metricKey)}">?</button>
    </div>
    ${desc ? `<div class="text-[11px] leading-[1.3] text-[color:var(--muted)]">${escapeHtml(desc)}</div>` : ''}
    ${metricObj?.ai_note ? `<div class="text-[11px] leading-[1.4] text-[color:var(--muted)] mt-1">${escapeHtml(metricObj.ai_note)}</div>` : ''}`;
...
function renderValueSmart(metricObj) {
  const v = metricObj?.raw ?? metricObj;
  if (v === null || v === undefined) return '—';
  if (typeof v === 'object') {
    try {
      const s = JSON.stringify(v);
      return `<code class="font-mono text-[12px]">${escapeHtml(s.length > 120 ? s.slice(0,120)+'…' : s)}</code>`;
    } catch { return '—'; }
  }
...
/* -----------------------------------------------------------------------------
   NORMALIZZAZIONE DATI (solo mapping; nessuna narrativa di default)
----------------------------------------------------------------------------- */
  ...
  return {
    meta: {
      ...
      labels: src?.meta?.labels || {
        ...
        options_title: 'Options Overlay',
        pattern_title: 'Pattern MTF Board',
    ...
    dataset: {
      OCR_Conf:        src?.dataset?.OCR_Conf        || { raw:'—', tone:'neutral' },
      DataIntegrity:   src?.dataset?.DataIntegrity   || { raw:'—', tone:'neutral' },
      FeedSync:        src?.dataset?.FeedSync        || { raw:'—', tone:'neutral' },
      SessionScope:    src?.dataset?.SessionScope    || { raw:'—', tone:'neutral' },
      MissingCritical: Array.isArray(src?.dataset?.MissingCritical) ? src.dataset.MissingCritical : [],
      Notes_OCR:       src?.dataset?.Notes_OCR       || ''
    },
    ...
    options: (function(){
      ...
      return {
        ...
        EM_rows: Array.isArray(o.EM_rows) ? o.EM_rows : [],
        PCR_breakdown: o.PCR_breakdown || null,
        DealerGamma: o.DealerGamma || null,
        MaxPain: o.MaxPain || null,
        GEX: o.GEX || null,
        Skew_set: o.Skew_set || null,
        Skew_detail: o.Skew_detail || null,
        TermSlope: o.TermSlope || null,
        TermNotes: o.TermNotes || '',
        Flow: o.Flow || null,
        Gamma_ATM: o.Gamma_ATM || null,
        Vega_ATM: o.Vega_ATM || null,
        ai_note: o.ai_note || '',
    ...
    pattern_mtf:   src?.pattern_mtf   || { heatmap: [] },
    mtf_consolidation: src?.mtf_consolidation || {},
    governance: src?.governance || { AuditPathID:'—', QualityMetrics:{} }
  };
}
