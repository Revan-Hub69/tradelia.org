...
//
// F3 · Analisi Tecnica MTF (3–10 giorni) — versione “tecnico puro” (NO opzioni)
// Allineato a F1B/F2: card snapshot + drawer con sezioni, tabs desktop/mobile,
// metriche con info-btn (glossario), reset scroll, zero testo hard-coded (solo ID).
//
// Export:
      ...
      <header class="section-headline mb-4">
        <div class="section-head-left">
          ...
          <div class="section-desc text-[12px] text-[color:var(--muted)] leading-[1.45] mt-1">
            ${escapeHtml(d.meta.hero_intro || 'Lettura istituzionale W1→D1→H4→H1 e sintesi probabilistica. Nessun contenuto operativo.')}<br/>
...
export function bindCard(node, rawData, ctx = {}) {
  ...
  // Tooltip "?": collega i bottoni informativi alla libreria globale
...
/* -----------------------------------------------------------------------------
// DRAWER (9 sezioni) — scoped root + safe activation
// -----------------------------------------------------------------------------*/
function openF3DrawerPublic(d){
  ...
  setTimeout(()=>{
    ...
    // bind tabs (scoped a questo drawer)
    bindF3TabsPublic();
...
function buildF3SectionsPublic(d){
  ...
  // 7) Pattern MTF Board — qualitativo
  ...
  // 8) Consolidamento MTF & Probabilità — quantitativo
  ...
  const mtfHTML = `
    <section class="tl-panel-section" data-f3-section="mtf" style="background:transparent;border:0;border-radius:0;box-shadow:none;padding:0;">
      ...
      <div class="grid md:grid-cols-2 gap-3 text-[12px] leading-[1.4]">
        ${metricBlockF3('COMP_MTF_info','Compositi MTF (Vol/Price)','', mtf?.Composites_MTF)}
  ...
  return { datasetHTML, w1HTML, d1HTML, h4HTML, h1HTML, priceHTML, patternHTML, mtfHTML, governanceHTML };
}
...
/* -----------------------------------------------------------------------------
// NORMALIZZAZIONE DATI (F3 — tecnico puro)
// -----------------------------------------------------------------------------*/
function normalizeDataF3Public(src = {}){
  ...
  const mtf_consolidation = (function(){
    ...
    return {
      Composites_MTF: m.Composites_MTF || { raw:{ Volume:'—', Price:'—' }, tone:'neutral' },
  ...
  return { meta, head, dataset, W1, D1, H4, H1, price_history, pattern_mtf, mtf_consolidation, governance };
}
