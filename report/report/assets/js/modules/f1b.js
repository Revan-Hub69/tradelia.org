...
export function renderCard(rawData, ctx = {}) {
  ...
  // Se ci sono rows formattate (F1A format), usa F1A (header-ticker) rendering
  const formattedRows = rawData?.rows || rawData?.formattedRows;
  if (formattedRows && Array.isArray(formattedRows) && formattedRows.length > 0) {
    // Restituisce un container per F1A (header-ticker) che verrà montato in bindCard
    return `
      <section class="f1b-card-container text-[13px] leading-[1.5] text-[color:var(--ink)]"
        ...
        <div class="f1b-header-ticker-container" data-f1b-header-ticker="true"></div>
      </section>
    `;
  }
...
export function bindCard(node, rawData, ctx = {}) {
  ...
  // Se ci sono rows formattate, monta F1A (header-ticker)
  const formattedRows = rawData?.rows || rawData?.formattedRows;
  const tickerContainer = node.querySelector('[data-f1b-header-ticker="true"]');
  if (formattedRows && Array.isArray(formattedRows) && formattedRows.length > 0 && tickerContainer) {
    // Importa e monta F1A (header-ticker component)
    import('../components/header-ticker.js').then(({ headerTicker }) => {
      const f1aNode = headerTicker.mount(tickerContainer);
      if (f1aNode) {
        // Prepara dati per F1A (stesso formato di header.json)
        const f1aData = {
          ...rawData.meta,
          rows: formattedRows,
          metricsPanel: rawData?.metricsPanel || []
        };
        headerTicker.update(f1aNode, f1aData);
      }
    }).catch(err => {
      console.warn('F1B: Errore caricamento F1A (header-ticker)', err);
    });
  }
...
function buildDrawerSectionsPublic(d) {
  ...
  const sintesiHTML = `
    <section class="tl-panel-section" data-f1b-section="sintesi"
      style="background:transparent;border:0;border-radius:0;box-shadow:none;padding:0;">
      <header class="tl-panel-section-title">
        <div class="tl-panel-section-title-text">Conclusione · Framework Accademico AI, Tradelia Swing Master 5.0 (educational)</div>
      </header>
