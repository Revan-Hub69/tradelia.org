...
const DEFAULT_REPORT_TYPE = 'swing_master_5_0';
    ...
    const REPORT_TEMPLATES = {
      swing_master_5_0: {
        label: 'SRD v5.0 ��� Swing Research Deck',
      ...
      daily_market_intel_3_1: {
        label: 'MTB v3.1 ��� Macro Tactical Briefing',
        description: 'Deck macro cross-asset in rollout',
        modules: []
      },
      custom: {
        label: 'CRD ��� Custom Research Deck',
    ...
    const setSavingState = (saving, mode) => {
      ...
      toggleButtonLoading(targetButton, saving, mode === 'publish' ? 'Pubblicazione�Ǫ' : 'Salvataggio�Ǫ');
      if (saving) {
    ...
    const buildTitleSuggestion = (date = new Date()) => {
      ...
      return `SRD v5.0 -� ${formatter.format(date)}`;
    };
    ...
    const updateEditorBadge = () => {
      if (isDirty) {
        ...
        editorReportIdEl.textContent = `${activeReport.slug} -� id ${activeReport.id.slice(0, 8)}�Ǫ`;
      } else {
      ...
      for (const card of cards) {
        ...
        if (seenKeys.has(key)) {
          ...
          setStatusBannerMessage(
            'error',
            'Chiave duplicata',
            `La chiave ${key} +� gi+� utilizzata. Usa identificativi distinti per ogni modulo.`
          );
          return null;
        }
        seenKeys.add(key);
    ...
    const handleChartDrop = async (event) => {
      ...
      if (files && files.length) {
        const file = files[0];
          ...
          showToast('Il file trascinato non +� un���immagine', 'error');
        }
              return;
            }
    ...
    const updateAuthUI = async () => {
      ...
      if (currentUser) {
        ...
        if (!allowed) {
          // Se non +� admin, mostra messaggio chiaro e redirect dopo 3 secondi
          ...
          authCard.innerHTML = `
            ...
            <p style="color: var(--ink-soft); margin: 1rem 0;">
              L'email <strong>${currentUser.email}</strong> non +� autorizzata ad accedere alla dashboard report.
    ...
    const ensureAdminAccess = async () => {
      ...
      try {
        ...
        if (!adminEmailData) {
          // Fallback: verifica anche admin_users se abbiamo user_id (retrocompatibilit+�)
          if (currentUser.id) {
    ...
    reportTypeEl.addEventListener('change', () => {
      ...
      if (template.modules.length === 0) {
        showToast('Questo template sar+� disponibile a breve', 'info');
        reportTypeEl.value = currentTemplateType;
        return;
      }
    ...
    document.addEventListener('paste', async (event) => {
      ...
      if (isEditableTarget) return; // i campi gestiscono gi+� la loro logica
