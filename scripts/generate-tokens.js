...
function generateCSS(tokens) {
  ...
  if (flat['typography-fontFamily-sans']) cssVars.push(`  --ff-sans: ${flat['typography-fontFamily-sans']};`);
  if (flat['typography-fontFamily-mono']) cssVars.push(`  --ff-mono: ${flat['typography-fontFamily-mono']};`);
  
  // Font sizes
  for (let i = 11; i <= 22; i++) {
    if (i === 17 || i === 19 || i === 21) continue; // Skip non-existent sizes
    const key = `typography-fontSize-${i}`;
    if (flat[key]) cssVars.push(`  --fs-${i}: ${flat[key]};`);
  }
  
  // Line heights
  for (const lh of ['1', '12', '13', '14', '15', '16', '17']) {
    const key = `typography-lineHeight-${lh}`;
    if (flat[key]) cssVars.push(`  --lh-${lh}: ${flat[key]};`);
  }
  ...
  for (const sh of ['sm', 'md', 'lg', 'xl', 'card']) {
    const key = `shadow-${sh}`;
    if (flat[key]) cssVars.push(`  --shadow-${sh}: ${flat[key]};`);
  }
  
  cssVars.push('');
  cssVars.push('  /* Container */');
  if (flat['container-max']) cssVars.push(`  --container-max: ${flat['container-max']};`);
  
  cssVars.push('');
  cssVars.push('  /* Breakpoints */');
  for (const bp of ['sm', 'md', 'lg', 'xl', '2xl']) {
    const key = `breakpoint-${bp}`;
    if (flat[key]) cssVars.push(`  --breakpoint-${bp}: ${flat[key]};`);
  }
  ...
  for (const tr of ['fast', 'base', 'slow']) {
    ...
    if (flat[key]) cssVars.push(`  --transition-${tr}: ${flat[key]};`);
  }
  ...
  if (flat['colors-text-ink']) cssVars.push(`  --ink: ${flat['colors-text-ink']};`);
  if (flat['colors-text-ink-soft']) cssVars.push(`  --ink-soft: ${flat['colors-text-ink-soft']};`);
  if (flat['colors-text-muted']) cssVars.push(`  --muted: ${flat['colors-text-muted']};`);
  if (flat['colors-text-muted-strong']) cssVars.push(`  --muted-strong: ${flat['colors-text-muted-strong']};`);
  
  cssVars.push('');
  cssVars.push('  /* Superfici */');
  if (flat['colors-surface-page']) cssVars.push(`  --surface-page: ${flat['colors-surface-page']};`);
  if (flat['colors-surface-card']) cssVars.push(`  --surface-card: ${flat['colors-surface-card']};`);
  if (flat['colors-surface-elev']) cssVars.push(`  --surface-elev: ${flat['colors-surface-elev']};`);
  if (flat['colors-surface-hover']) cssVars.push(`  --surface-hover: ${flat['colors-surface-hover']};`);
  if (flat['colors-surface-card-alt']) cssVars.push(`  --surface-card-alt: ${flat['colors-surface-card-alt']};`);
  if (flat['colors-surface-panel-head']) cssVars.push(`  --surface-panel-head: ${flat['colors-surface-panel-head']};`);
  
  cssVars.push('');
  cssVars.push('  /* Bordi */');
  if (flat['colors-border-soft']) cssVars.push(`  --br-soft: ${flat['colors-border-soft']};`);
  if (flat['colors-border-card']) cssVars.push(`  --br-card: ${flat['colors-border-card']};`);
  if (flat['colors-border-strong']) cssVars.push(`  --br-strong: ${flat['colors-border-strong']};`);
  if (flat['colors-border-panel-divider']) cssVars.push(`  --br-panel-divider: ${flat['colors-border-panel-divider']};`);
  
  cssVars.push('');
  cssVars.push('  /* Footer */');
  if (flat['colors-footer-bg']) cssVars.push(`  --footer-bg: ${flat['colors-footer-bg']};`);
  if (flat['colors-footer-br']) cssVars.push(`  --footer-br: ${flat['colors-footer-br']};`);
  
  cssVars.push('');
  cssVars.push('  /* Brand */');
  if (flat['colors-brand-600']) cssVars.push(`  --brand-600: ${flat['colors-brand-600']};`);
  if (flat['colors-brand-500']) cssVars.push(`  --brand-500: ${flat['colors-brand-500']};`);
  if (flat['colors-brand-400']) cssVars.push(`  --brand-400: ${flat['colors-brand-400']};`);
  
  cssVars.push('');
  cssVars.push('  /* Stati */');
  if (flat['colors-status-ok']) cssVars.push(`  --ok: ${flat['colors-status-ok']};`);
  if (flat['colors-status-warn']) cssVars.push(`  --warn: ${flat['colors-status-warn']};`);
  if (flat['colors-status-err']) cssVars.push(`  --err: ${flat['colors-status-err']};`);
  if (flat['colors-status-neutral']) cssVars.push(`  --neutral: ${flat['colors-status-neutral']};`);
  
  cssVars.push('');
  cssVars.push('  /* Tone Colors */');
  if (flat['colors-tone-pos-fg']) cssVars.push(`  --tone-pos-fg: ${flat['colors-tone-pos-fg']};`);
  if (flat['colors-tone-neg-fg']) cssVars.push(`  --tone-neg-fg: ${flat['colors-tone-neg-fg']};`);
  if (flat['colors-tone-warn-fg']) cssVars.push(`  --tone-warn-fg: ${flat['colors-tone-warn-fg']};`);
  if (flat['colors-tone-neu-fg']) cssVars.push(`  --tone-neu-fg: ${flat['colors-tone-neu-fg']};`);
