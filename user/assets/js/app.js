...
function selectProfileTab(tab) {
  const tabMap = {
    main: {
      tab: PROFILE_TAB_MAIN,
      blocks: [PROFILE_BLOCK_MAIN, PROFILE_BUSINESS_SECTION, PROFILE_BLOCK_PREFERENCES],
    },
...
async function handleCancelSubscription() {
  if (
    !confirm(
      "Sei sicuro di voler cancellare il tuo abbonamento? L'accesso verrà disattivato alla scadenza del periodo pagato."
    )
  ) {
    return;
  }
  ...
  const businessData =
    user_type === "business"
      ? {
          ...
          business_invoice_days: PROFILE_BUSINESS_INVOICE_DAYS?.value
            ? parseInt(PROFILE_BUSINESS_INVOICE_DAYS.value)
            : 0,
          ...
          business_contact_email:
            PROFILE_BUSINESS_CONTACT_EMAIL?.value.trim().toLowerCase() || null,
...
function renderCreditsHistory() {
  ...
  CREDITS_HISTORY.innerHTML = state.creditsLog
    .map((entry) => {
...
async function renderProposalsList() {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        PROPOSAL_LIST.innerHTML =
          '<p style="color: rgba(203, 213, 225, 0.6); font-size: 0.9rem;">Nessuna richiesta ancora. Invia un ticker per richiedere un\'analisi.</p>';
        return;
      }
      ...
      PROPOSAL_LIST.innerHTML = data
        .map((request) => {
          ...
          const reportLink =
            request.report_slug || request.report_id
  ...
  PROPOSAL_LIST.innerHTML =
    '<p style="color: rgba(203, 213, 225, 0.6); font-size: 0.9rem;">Invia un ticker per richiedere un\'analisi.</p>';
}
...
function renderCommunityProposalsList() {
  ...
  COMMUNITY_PROPOSALS_LIST.innerHTML = sortedProposals
    .map((proposal) => {
...
async function renderRecentReports() {
  ...
  try {
    if (!state.user) {
      container.innerHTML =
        '<div class="empty-state"><p>Nessun report completato ancora.</p></div>';
      return;
    }
...
async function renderReportsSection() {
  ...
  try {
    if (!state.user) {
      container.innerHTML =
        '<div class="empty-state"><p>Effettua l\'accesso per vedere i tuoi report.</p></div>';
      return;
    }
